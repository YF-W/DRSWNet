...
class AdaptiveDirectionalFilter(nn.Module):
    def __init__(self, num_directions=4):
        ...
        self.theta = nn.Parameter(torch.linspace(0, 2 * np.pi, num_directions))  # 初始化多方向 theta，角度为弧度
        self.bandwidth = nn.Parameter(torch.tensor(30.0))  # 初始化带宽为 30，可学习参数
...
class CCFEA_Module(nn.Module):
    def __init__(self, in_channels, reduction_ratio=16, expansion_ratio=4):
...
class WP_MFP(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(WP_MFP, self).__init__()
...
class MDDGNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=1, small_kernel=5, small_padding=2, channels=[32, 64, 128, 256, 512]):
        super(MDDGNet, self).__init__()import torch
from torch import einsum, nn
import torch.nn.functional as F
from einops import rearrange, repeat
from einops.layers.torch import Rearrange
...
class MDDGNet(nn.Module):
    def __init__(self, in_channels=3, out_channels=1, small_kernel=5, small_padding=2, channels=[32, 64, 128, 256, 512]):
        ...
        self.down_2_1 = WP_MFP(channels[0], channels[0])
        self.dc_2_2 = ResidualBlock(channels[0], channels[1])
        self.down_2_2 = WP_MFP(channels[1], channels[1])
        self.dc_2_3 = ResidualBlock(channels[1], channels[2])
        self.down_3_2 = WP_MFP(channels[2], channels[2])
        ...
        self.down_3_3 = WP_MFP(channels[2], channels[2])
        self.down_3_4 = WP_MFP(channels[2], channels[2])
